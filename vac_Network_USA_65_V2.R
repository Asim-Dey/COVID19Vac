...
Motif_concentration2
 ...
 Motif_counts2/edge
# 2.885325  5.537837 52.259989  0.416592 13.298274  7.134026
...
meanD1<-mean(degree1);meanD1 #10.4906
sdD1<-sd(degree1);sdD1 # 7.765356
...
AVPL<-average.path.length(G0);AVPL # 17.72673
Diameter<-diameter(G0) ;Diameter # 58
...
assortativity_degree(G11)  #  0.9135765

assortativity(G11, V(G11)$vac18, directed=F) #  0.4810483


assortativity_nominal(G11, as.integer(as.factor(V(G11)$state), directed=F)) #  0.7861316


assortativity(G11, V(G11)$Covid_cases, directed=F) # 0.3606316


assortativity(G11, V(G11)$Edu_ass_coll, directed=F) # 0.5009849

assortativity(G11, V(G11)$Income, directed=F) # 0.5643259
