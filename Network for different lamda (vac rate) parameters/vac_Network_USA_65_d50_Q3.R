...
k2=70.80   # 3rd Qu. or above of 18 netowrk for comparison
...
#write.csv(edlist_65_Dist, '~/Library/CloudStorage/OneDrive-Personal/COVID-19 Vac Network/Data and Code/Data/COVID-19 Vaccinations in the US/edlist_65_Dist_d50_3Q.csv')

############################## Motif ###############################################
#---------------------------------------

node<-length(V(G0));node # 3192
edge<-length(E(G0));edge # 13174
...
meanD1<-mean(degree1);meanD1 # 8.254386
sdD1<-sd(degree1);sdD1 # 7.421488
...
plot(degree_D65, type="o", lwd=2,xaxt="no", ylab = "p(k)",xlab = "Number of edge (Degree), k",
     main = "",ylim = c(),col="blue",first.pannel=grid())
...
AVPL<-average.path.length(G0);AVPL # 18.27372
Diameter<-diameter(G0) ;Diameter # 59
...
assortativity_degree(G11)  #  0.905957

assortativity(G11, V(G11)$vac18, directed=F) #  0.4731532


assortativity_nominal(G11, as.integer(as.factor(V(G11)$state), directed=F)) #  0.7824935


assortativity(G11, V(G11)$Covid_cases, directed=F) # 0.3800782


assortativity(G11, V(G11)$Edu_ass_coll, directed=F) # 0.5162314

assortativity(G11, V(G11)$Income, directed=F) # 0.5681322
