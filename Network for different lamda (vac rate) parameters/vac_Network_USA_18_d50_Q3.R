...
edge_density(G0, loops=F) # 0.0004561309
...
#write.csv(edlist_18_Dist, '~/Library/CloudStorage/OneDrive-Personal/COVID-19 Vac Network/Data and Code/Data/COVID-19 Vaccinations in the US/edlist_18_Dist_d50_3Q.csv')
...
meanD1<-mean(degree1);meanD1 # 1.455514
sdD1<-sd(degree1);sdD1 # 4.251557
...
AVPL<-average.path.length(G0);AVPL # 6.593763
Diameter<-diameter(G0) ;Diameter # 22
...
assortativity_degree(G11) # 0.8973382




assortativity(G11, V(G11)$vac18, directed=F) #  0.3605779


assortativity_nominal(G11, as.integer(as.factor(V(G11)$state), directed=F)) #  0.6951807


assortativity(G11, V(G11)$Covid_cases, directed=F) # 0.5865162



assortativity(G11, V(G11)$Edu_ass_coll, directed=F) # 0.5630132

assortativity(G11, V(G11)$Income, directed=F) # 0.4678421
