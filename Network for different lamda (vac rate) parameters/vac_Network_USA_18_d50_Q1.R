...
edge_density(G0, loops=F) # 0.002247667
...
#write.csv(edlist_18_Dist, '~/Library/CloudStorage/OneDrive-Personal/COVID-19 Vac Network/Data and Code/Data/COVID-19 Vaccinations in the US/edlist_18_Dist_d50_1Q.csv')
...
meanD1<-mean(degree1);meanD1 # 7.172306
sdD1<-sd(degree1);sdD1 # 7.14139
...
AVPL<-average.path.length(G0);AVPL # 18.71781
Diameter<-diameter(G0) ;Diameter # 62
...
assortativity_degree(G11) # 0.9026432




assortativity(G11, V(G11)$vac18, directed=F) #  0.4650024


assortativity_nominal(G11, as.integer(as.factor(V(G11)$state), directed=F)) #  0.7808457


assortativity(G11, V(G11)$Covid_cases, directed=F) # 0.407669



assortativity(G11, V(G11)$Edu_ass_coll, directed=F) # 0.5368768

assortativity(G11, V(G11)$Income, directed=F) # 0.5846846
